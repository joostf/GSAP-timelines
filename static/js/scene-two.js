...
const tlSceneTwo = gsap.timeline({ defaults: { duration: 1 }, paused:true})
...
function tlPlanet() {
   ...
   tl.to(DOM.planet, 1, { scale: 6, yPercent: 120 })
...
function tlRocket() {
   const tl = gsap.timeline({})
   tl.to(DOM.rocket, 1, { scale: .3, transformOrigin: '50% 100%' })
...
function tlSpaceNerd() {
   ...
   tl.to(DOM.spaceNerd, 1, { opacity: 1, xPercent:20, yPercent: -20, scale: .1 })
...
function tlSpaceWelder () {
   ...
   tl.set(DOM.spaceWelder, {xPercent: '62.5'})
